...
# 为每个测试用例创建一个临时配置
test_config = Config(SOURCE_DIR=str(project_base / "qemu_m0"), BUILD_DIR=tempfile.mkdtemp(),
                     uuid=str(uuid.uuid4()))
...
class TestAutomationDebugger(unittest.TestCase):
    ...
    def test_02_build(self):
        ...
        # 验证编译对象已生成
        self.assertTrue(
            os.path.exists(os.path.join(test_config.BUILD_DIR, f"build-{test_config.uuid}", test_config.TARGET_NAME)))
